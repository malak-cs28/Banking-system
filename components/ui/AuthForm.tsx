...
const formSchema = (type: string) =>
  z.object({
    ...
    address1:
      type === "sign-in"
        ? z.string().optional().nullable()
        : z.string().max(50, { message: "Address must be at most 50 characters" }),
    state:
      type === "sign-in"
        ? z.string().optional().nullable()
        : z
            .string()
            .length(2, { message: "State must be 2 letters" })
    ...
    postalCode:
      type === "sign-in"
        ? z.string().optional().nullable()
        : z.string().regex(/^\d{5}(-\d{4})?$/, { message: "Postal code must be a valid US ZIP code" }),
    dateOfBirth:
      type === "sign-in"
        ? z.string().optional().nullable()
        : z.string().regex(/^\d{4}-\d{2}-\d{2}$/, { message: "Date of Birth must be in YYYY-MM-DD format" }),
    ssn:
      type === "sign-in"
        ? z.string().optional().nullable()
        : z.string().regex(/^\d{3}-\d{2}-\d{4}$|^\d{9}$/, { message: "SSN must be valid format" }),
  });
  ...
  const onSubmit = async (values: z.infer<ReturnType<typeof formSchema>>) => {
    ...
    try {
      if (type === "sign-up") {
        ...
        const formattedDob = formatDateOfBirth(values.dateOfBirth!);
        const formattedState = values.state?.toUpperCase().slice(0, 2);
        const formattedPostalCode = values.postalCode!;
        const formattedSsn = values.ssn!.replace(/\D/g, "");
        ...
        <div className="flex flex-col items-start gap-4 w-full">
          <div className="flex items-center justify-center gap-3">
            <div className="relative w-12 h-12">
              <Image src="/icons/credit-card.svg" alt="Bank Logo" width={48} height={48} className="object-contain" />
            </div>
            <h2 className="text-24 lg:text-36 font-semibold text-gray-900">NU Bank</h2>
          </div>
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              {type === "sign-up" && (
                <>
                  ...
                  <div className="flex gap-4">
                    <FormInput control={form.control} name="state" label="State (e.g., CA)" placeholder="CA" />
                    <FormInput control={form.control} name="postalCode" label="Postal Code" placeholder="12345" />
                  </div>
                  <div className="flex gap-4">
                    <FormInput control={form.control} name="dateOfBirth" label="Date of Birth (YYYY-MM-DD)" placeholder="MM/DD/YYYY or YYYY-MM-DD" />
                    <FormInput control={form.control} name="ssn" label="SSN" placeholder="123-45-6789" />
                  </div>
                </>
              )}
